...
For ReferenceStripFeeder the ‘Direction of Travel’ and ‘Direction of Feed’ are confusing since nothing is being fed; the tape is fixed to a location.
 ...
 The tape can be in any orientation on the table but the direction of travel along the tape always follows this rule. For clarification, the image below shows tapes oriented at +90°, 180°, -90°, and 0° relative to the machine in that order left to right. It also shows where the first and second hole and their corresponding parts are located in the tape:
...
This error usually occurs when the strip feeder vision system cannot find the next tape hole. The most common causes of this problem are attempting to feed from the wrong end of the tape, incorrect units per pixel settings on the camera, or poor lighting.

First verify the tape strip orientation and first hole match that as shown above in [Tape Strip Orientation](https://github.com/openpnp/openpnp/wiki/ReferenceStripFeeder/_edit#tape-strip-orientation).
