...
Finally, when the Ok button is clicked, the panel array is added to the panel as shown below. Notice that the original board Id has been replicated to all boards in the array is appended with "[row#,col#]" to designate each element of the array:
